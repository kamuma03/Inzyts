...
from src.prompts import DIAGNOSTIC_STRATEGY_PROMPT


class DiagnosticStrategyAgent(BaseAgent):
    """
    Diagnostic Strategy Agent.
    Plans root cause analysis.
    """
    ...
    def process(self, state: AnalysisState, **kwargs) -> Dict[str, Any]:
        ...
        extension_output = state.diagnostic_extension
        ...
        response_str = self.llm_agent.invoke_with_json(
            prompt=f"Design diagnostic strategy for: {context}"
        )
        ...
        try:
            ...
            response_dict["analysis_type"] = AnalysisType.CAUSAL
