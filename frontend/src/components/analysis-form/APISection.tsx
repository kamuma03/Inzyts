        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
                ...
                {AUTH_OPTIONS.map(({ value, label }) => (
                    <button key={value} type="button" onClick={() => setApiAuthType(value)}
                        className={`py-[0.35rem] px-3 rounded border cursor-pointer text-[0.85rem] ${apiAuthType === value ? 'border-[var(--accent)] bg-[rgba(56,178,172,0.15)] text-[var(--accent)]' : 'border-[var(--rule)] bg-transparent text-[var(--text-secondary)]'}`}>
                        {label}
                    </button>
                ))}
            </div>
            {apiAuthType === 'bearer' && (
                <input type="password" placeholder="Bearer Token" value={apiAuthToken} onChange={(e) => setApiAuthToken(e.target.value)}
                    className="w-full p-3 rounded border border-[var(--rule)] bg-[rgba(0,0,0,0.2)] text-[var(--text-primary)]" />
            )}
            {apiAuthType === 'api_key' && (
                <div className="flex gap-2">
                    <input type="text" placeholder="Header Name (e.g. X-API-Key)" value={apiKeyName} onChange={(e) => setApiKeyName(e.target.value)}
                        className="flex-1 p-3 rounded border border-[var(--rule)] bg-[rgba(0,0,0,0.2)] text-[var(--text-primary)]" />
            ...
            <input
                ...
                className="w-full p-3 rounded border border-[var(--rule)] bg-[rgba(0,0,0,0.2)] text-[var(--text-primary)] text-[0.9rem]"
            />
